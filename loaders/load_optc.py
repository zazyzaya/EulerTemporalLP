...
def load_optc_list_dist(workers, start=0, end=1, delta=30):
    ...
    # Then execute all jobs
    snapshots = Parallel(n_jobs=workers, prefer='processes')(
        delayed(load_optc_list)(**kwargs[i]) for i in range(workers)
    )

    eis, ys = zip(*snapshots)
    return eis, ys


def load_optc_list(start=0, end=None, delta=30):
    '''
    Just return a list of all connections without condensing them into 
    graphs. Returns a list of all connections (with node ids)
    '''
    format_f = lambda x : DATA_FOLDER + str(x) + '.csv'
    ...
    while(not eof and stamp(line) < end):
        ...
        # Continue loading data until timewindow complete, or we reach the end
        while(not eof and stamp(line) < cur_delta + delta and stamp(line) < end):
            s, d = int(line[SRC]), int(line[DST])

            # Ignore self-loops
            if src != dst:
                labels.append(int(line[Y]))
                src.append(s)
                dst.append(d)
